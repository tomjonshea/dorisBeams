...
SimplePatternList gPatterns = { circularRainbow, sinelon, rainbow, rainbowWithGlitter, glitterOnly, sinelon, confetti, sinelon, bpm, sinelon };
...
void loop() {
    ...
    if (BRIGHTNESS_POTENTIOMETER_ON) {
        ...
        uint8_t brightnessModulator = map(brightnessPotentiometerValue, 0, BRIGHTNESS_POTENTIOMETER_MAX, 0, 255);
        FastLED.setBrightness(brightnessModulator/255 * FastLED.getBrightness());
    }
...
void moodLighting() {
    FastLED.setBrightness(64); //might not be necessary
